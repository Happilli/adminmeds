...
function StatCard({ number, title, subtitle, icon, percentage, trend = "up" }) {
    const isUp = trend === "up";
        ...
        <div className="bg-surface-container rounded-2xl p-6 border border-outline-variant
         hover:border-primary/40 transition-colors">
            <div className="flex justify-between items-start mb-5">
                <div className="w-12 h-12 rounded-xl bg-secondary-container flex items-center
                 justify-center text-secondary-fixed">
                    {icon}
                </div>
                {percentage && (
                    <div
                        className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1 ${isUp
